...
async function loadFromApi(){
  ...
  if(user){
    D.perfil = user;
    // Tenta renovar token se existir
    if(user.token){
      try {
  ...
  try{
    const [propostas, feed, lanches] = await Promise.all([
      apiFetch('/propostas'),
      apiFetch('/feed'),
      D.perfil ? apiFetch('/lanches') : Promise.resolve([])
    ]);
...
/* ── NAV ── */
// ✅ FIX 5: Cache de 30s para evitar reload em toda navegação
let lastReload = 0;
  ...
  el.innerHTML = D.lanches.map(l=>`
    <div class="lanche-edit-card">
      ...
      <div class="lanche-edit-body">
        ...
        ${l.descricao?`<div class="lanche-edit-desc">${l.descricao}</div>`:''}
        <div class="btn-row">
...
function abrirModalLanche(id){
  ...
  document.getElementById('ml-desc').value = l?.descricao||'';
...
// ✅ FIX 7: onFotoChange sem código duplicado
...
async function salvarLanche(){
  ...
  const descricao = document.getElementById('ml-desc').value.trim();
  ...
  try{
    if(lancheEditId){
      await apiFetch('/lanches/'+encodeURIComponent(lancheEditId),{
        method:'PUT',
        body:JSON.stringify({ nome, descricao, foto })
      });
    }else{
      await apiFetch('/lanches',{
        method:'POST',
        body:JSON.stringify({ nome, descricao, foto })
      });
    }
...
function propostasFiltradas(){
  ...
  return lista.filter(p=>{
    ...
    const descricao = (p.desc||'').toLowerCase();
    const autor = (p.autorNome||'').toLowerCase();
    return local.includes(q) || lanche.includes(q) || descricao.includes(q) || autor.includes(q);
  });
}
...
function abrirModalProp(id){
  ...
  document.getElementById('mp-lanche-preview').innerHTML = `
    <div style="background:var(--s2);border-radius:var(--rsm);padding:12px 14px;display:flex;gap:12px;align-items:center">
      ${p.foto?`<img src="${p.foto}" style="width:56px;height:56px;border-radius:8px;object-fit:cover" onerror="this.style.display='none'">`:
      '<div style="width:56px;height:56px;border-radius:8px;background:var(--s3);display:flex;align-items:center;justify-content:center;font-size:1.4rem">🍽️</div>'}
      <div>
...
async function abrirModalConversa(propId){
  ...
  document.getElementById('mc-preview').innerHTML = `
    <div style="background:var(--s2);border-radius:var(--rsm);padding:12px 14px;display:flex;gap:12px;align-items:center">
      ${p.foto?`<img src="${p.foto}" style="width:48px;height:48px;border-radius:8px;object-fit:cover" onerror="this.style.display='none'">`:
      '<div style="width:48px;height:48px;border-radius:8px;background:var(--s3);display:flex;align-items:center;justify-content:center;font-size:1.2rem">🍽️</div>'}
      <div>
        <div style="font-weight:700;font-size:.95rem">${p.lanche}</div>
        <div style="font-size:.8rem;color:var(--ink-mid)">${p.autorNome} · ${p.autorLocal}</div>
      </div>
    </div>`;
  const msgs = meuInteresse.mensagens || (meuInteresse.mensagem ? [{ autor: meuInteresse.nome, texto: meuInteresse.mensagem, data: '' }] : []);
...
async function recusarInteresse(propId, interessadoUsuarioId){
  ...
  if(!p) return;
  try{
    // ✅ FIX 9: rota corrigida de /recusar para /rejeitar
    await apiFetch('/propostas/'+encodeURIComponent(propId)+'/rejeitar',{
      method:'POST',
...
function cardFeed(f){
  const euId = D.perfil?.id;
  const participa = euId && (f.aId===euId || f.bId===euId);

  const reacoes = Object.entries(f.reacoes || {}).map(([emoji,lista])=>{
    const ativa = euId && (Array.isArray(lista)?lista:[]).includes(euId) ? 'ativa' : '';
    return `<button class="reacao-btn ${ativa}" onclick="reagir('${escId(f.id)}','${String(emoji).replace(/'/g,"\\'")}')">
      ${emoji} <span class="reacao-count">${(lista&&lista.length)||''}</span>
    </button>`;
  }).join('');
